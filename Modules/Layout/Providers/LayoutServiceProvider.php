<?php

namespace Modules\Layout\Providers;

use Illuminate\Support\ServiceProvider;
use Modules\Layout\Repositories\ExtensionRepository;
use Modules\Layout\Repositories\Interfaces\ExtensionRepositoryInterface;
use Modules\Layout\Repositories\Interfaces\LayoutModuleRepositoryInterface;
use Modules\Layout\Repositories\Interfaces\LayoutRepositoryInterface;
use Modules\Layout\Repositories\Interfaces\ModuleRepositoryInterface;
use Modules\Layout\Repositories\LayoutModuleRepository;
use Modules\Layout\Repositories\LayoutRepository;
use Modules\Layout\Repositories\ModuleRepository;

class LayoutServiceProvider extends ServiceProvider
{
    /**
     * @var string
     */
    protected $moduleName = 'Layout';

    /**
     * @var string
     */
    protected $moduleNameLower = 'layout';

    /**
     * Boot the application events.
     *
     * @return void
     */
    public function boot()
    {
        $this->registerTranslations();
        $this->registerConfig();
        $this->registerViews();
        $this->loadMigrationsFrom(module_path($this->moduleName, 'Database/Migrations'));
    }

    /**
     * Register the service provider.
     *
     * @return void
     */
    public function register()
    {
        $this->app->bind(LayoutRepositoryInterface::class, LayoutRepository::class);
        $this->app->bind(ModuleRepositoryInterface::class, ModuleRepository::class);
        $this->app->bind(ExtensionRepositoryInterface::class, ExtensionRepository::class);
        $this->app->bind(LayoutModuleRepositoryInterface::class, LayoutModuleRepository::class);
        $this->app->register(RouteServiceProvider::class);
    }

    /**
     * Register config.
     *
     * @return void
     */
    protected function registerConfig()
    {
        $this->publishes([
            module_path($this->moduleName, 'Config/config.php') => config_path($this->moduleNameLower . '.php'),
        ], 'config');
        $this->mergeConfigFrom(
            module_path($this->moduleName, 'Config/config.php'),
            $this->moduleNameLower
        );
    }

    /**
     * Register views.
     *
     * @return void
     */
    public function registerViews()
    {
        $viewPath = resource_path('views/modules/' . $this->moduleNameLower);

        $sourcePath = module_path($this->moduleName, 'Resources/views');

        $this->publishes([
            $sourcePath => $viewPath,
        ], ['views', $this->moduleNameLower . '-module-views']);

        $this->loadViewsFrom(array_merge($this->getPublishableViewPaths(), [$sourcePath]), $this->moduleNameLower);
    }

    /**
     * Register translations.
     *
     * @return void
     */
    public function registerTranslations()
    {
        $langPath = resource_path('lang/modules/' . $this->moduleNameLower);

        if (is_dir($langPath)) {
            $this->loadTranslationsFrom($langPath, $this->moduleNameLower);
        } else {
            $this->loadTranslationsFrom(module_path($this->moduleName, 'Resources/lang'), $this->moduleNameLower);
        }
    }

    /**
     * Get the services provided by the provider.
     *
     * @return array
     */
    public function provides()
    {
        return [];
    }

    private function getPublishableViewPaths(): array
    {
        $paths = [];
        foreach (\Config::get('view.paths') as $path) {
            if (is_dir($path . '/modules/' . $this->moduleNameLower)) {
                $paths[] = $path . '/modules/' . $this->moduleNameLower;
            }
        }

        return $paths;
    }
}
