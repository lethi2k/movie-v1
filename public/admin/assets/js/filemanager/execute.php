...
if (isset($info['extension']) && ! (isset($_GET['action'])
    && $_GET['action'] == 'delete_folder') && ! check_extension($info['extension'], $config)
    && $_GET['action'] != 'create_file' && ! (is_dir($path) && in_array($_GET['action'], ['rename_folder', 'create_folder', 'paste_clipboard']))
) {
    ...
    switch ($_GET['action']) {
        ...
        case 'create_folder':
            if ($config['create_folders']) {
                $name = fix_filename($_POST['name'], $config, true);
                $path .= $name;
                $path_thumb .= $name;
                $res = create_folder(fix_path($path, $config, true), fix_path($path_thumb, $config, true), $ftp, $config);
                if (! $res) {
