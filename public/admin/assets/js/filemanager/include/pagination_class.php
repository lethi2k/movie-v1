<?php
    ...
    class pagination
    {
         ...
         * @access public
         */
        public function __construct($array, $curPage = null, $perPage = null, $show_limit = 3)
        {
            $this->array = $array;
            $this->curPage = ($curPage == null ? $this->defaults['page'] : $curPage);
            $this->perPage = ($perPage == null ? $this->defaults['perPage'] : $perPage);
            $this->show_limit = ($show_limit == null ? $this->defaults['show_limit'] : $show_limit);
        }
         ...
         * links
         *
         * @param bool $showFirstAndLast True to show, false to hide.
         * @return void
         * @access public
         */
        ...
        public function getLinks($params = [])
        {
            ...
            if (($this->pages) > 1) {
                ...
                if ($this->page > $this->show_limit + 1) {
                    $links[] = ' <a>...</a> ';
                }
                // Assign all the page numbers & links to the array

                $p_start = (($this->page - $this->show_limit) > 1) ? ($this->page - $this->show_limit) : 1;
                $p_end = (($this->page + $this->show_limit) < ($this->pages + 1)) ? ($this->page + $this->show_limit) : ($this->pages + 1);
                ...
                if ($this->page < $this->pages - ($this->show_limit + 1)) {
                    $links[] = ' <a>...</a> ';
                }
