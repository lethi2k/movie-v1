...
{
    $str = sanitize($str);
    if ($config['convert_spaces'] && $is_folder === false) {
        $str = str_replace(' ', $config['replace_with'], $str);
    }

    if ($config['transliteration'] && $is_folder === false) {
...
function fix_path($path, $config, $is_folder = false)
{
    $info = pathinfo($path);
    $tmp_path = $info['dirname'];
    $str = fix_filename($info['basename'], $config, $is_folder);
    if ($tmp_path != "") {
